...
export function TaskProgressMessage({ message }: { message: AgentMessage }) {
  ...
  return (
    <div
      className={cn(
        'rounded-md bg-card border overflow-hidden transition-colors duration-300',
        isError && 'border-red-500/30',
        isStopped && 'border-yellow-500/30',
        isDone && 'border-green-500/20',
        isRunning && 'border-blue-500/30',
        !isError && !isStopped && !isRunning && !isDone && 'border-border/50'
      )}
    ...
    >
      ...
      >
        {/* Terminal icon — color matches status */}
        <svg className={cn('h-3 w-3 shrink-0', isDone ? 'text-green-400' : isError ? 'text-red-400' : 'text-blue-400')} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        <span className="ml-auto flex items-center gap-1.5 shrink-0">
          ...
          {elapsedStr && (
            <span className={cn('text-[10px]', isRunning ? 'text-blue-400/70 tabular-nums' : 'text-muted-foreground')}>
              {elapsedStr}
            </span>
          )}
          {isRunning && (
        ...
        <span className={cn(
          'text-muted-foreground transition-transform duration-200 text-[10px]',
          expanded && 'rotate-90'
        )}>▶</span>
      </button>
      {isRunning && (
        ...
        <div className="border-t border-border/30 px-3 py-2 space-y-2">
          <div className="flex items-center gap-2 text-[10px] text-muted-foreground font-mono">
            <span className={cn(
              ...
              !isDone && !isError && !isStopped && !isRunning && 'bg-muted-foreground'
            )} />
