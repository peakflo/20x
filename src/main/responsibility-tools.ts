...
const rootTools: Tool[] = [
  ...
  },
  {
    ...
    inputSchema: { type: 'object', additionalProperties: false, properties: { humanInputId: string, title, summary, basedOn: string, agentId: workAgent, groupId: { type: ['string', 'null'] } }, required: ['humanInputId', 'title'] }
  },
  {
    ...
    inputSchema: { type: 'object', properties: { humanInputId: string, title, summary, basedOn: string, factoryId: string, agentId: workAgent, groupId: { type: ['string', 'null'] } }, required: ['humanInputId', 'title'] }
  },
  {
  ...
  contextTool, resultTool, factoryTool,
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        action: { type: 'string', enum: ['done', 'continue', 'ask', 'ignore', 'notify', 'task', 'complete'], description: 'Goal work: done/continue/ask. Other work: done/ask. Verification: done/continue/ask. Source classification: ignore/notify/ask/task. Factory coordination: task/done/ask. Setup: propose a Routine, then done/ask. Work or classification may use complete only for a Routine with approved stopOnSuccess, requesting independent verification before monitoring stops.' },
        ...
        workItem: { type: 'object', additionalProperties: false, properties: { key: { ...string, maxLength: 400, description: 'Stable semantic identity within this execution; reuse it for later tasks on the same item.' }, title: { ...string, maxLength: 120, description: 'Short human-readable item title.' }, groupName: { ...string, maxLength: 120, description: 'Optional presentation name for this item Group.' } }, required: ['key', 'title'], description: 'Optional for Factory task selection or Goal work continuation. Distinct items get distinct Groups without domain-specific ticket/log assumptions.' },
...
export async function callResponsibilityTool(manager: ResponsibilityManager, token: string, name: string, args: Record<string, unknown> = {}) {
  try {
    ...
    switch (name) {
      ...
      case 'inspect_groups': result = await manager.controlTasks(scope, args, true, 'group'); break
