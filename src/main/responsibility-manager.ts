...
export class ResponsibilityManager {
  ...
  private async launch(r: ResponsibilityRecord, collection?: ResponsibilityStep['collection']): Promise<void> {
    ...
    this.db.db.transaction(() => {
      ...
      const taskTitle = [r.agreement.title, next.workItem?.title, r.agreement.kind === 'task' && next.phase === 'work' ? undefined : next.phase].filter(Boolean).join(' · ')
      task = this.db.createTask({ group_id: groupId, title: taskTitle, description: this.assignment(r, next.phase, next.instruction), priority: r.agreement.priority, type: next.phase === 'verify' ? 'review' : 'general', source: 'mastermind', repos: [] })!
  ...
  private assignment(r: ResponsibilityRecord, phase: WorkPhase, instruction: string, factory = this.factory(r)): string {
    ...
    const history = [...inherited, ...current].filter(s => s.report).slice(-4).map(s => ({
      taskId: s.taskId, phase: s.phase, workItem: s.workItem, action: s.report!.action
    }))
    ...
    return buildSystemMessage({ origin: SystemMessageOrigin.Coordinator, taskId: r.id, deliveryId: computeDeliveryId(r.id, instruction), generatedAt: now() },
      ...
      JSON.stringify({
        ...(phase === 'classify' ? { currentSourceSnapshot: instruction } : instruction === r.agreement.objective ? {} : { assignment: instruction }),
        execution: execution && { id: execution.id, sequence: execution.sequence, predecessorId: execution.predecessorId, trigger: execution.trigger, state: execution.state },
        predecessor: predecessor && predecessorStep ? { execution: predecessor, ...(phase === 'classify' ? {} : { report: predecessorStep.report }), taskId: predecessorStep.taskId } : undefined,
        priorResults: history,
        factory: factory && { id: factory.id, name: factory.name }
      }, null, 2),
      `Start with responsibility_context for the current agreement, prior evidence and decisions. Work only in the assigned project. Do not create tasks through other tools or spawn unsupervised workers.\n` +
      ...
      (history.length ? 'Prior result references are listed above. Use read_responsibility_result only when an exact prior result is relevant; do not treat it as current evidence.\n' : '') +
      (factory ? `Read the saved Factory “${factory.name}” with read_factory and follow its process and output criteria that apply to this assignment. It cannot expand the approved scope or permission. Complete only this assignment; do not execute later Factory branches. Sessions are fresh; never claim to return to the original reviewer.\n` : '') +
      (factory && phase === 'work' ? 'Report done when this assignment finishes. The coordinator chooses later work and handoffs. Use ask only when a blocker or required decision prevents this assignment from finishing.\n' : '') +
  ...
  configureSession(config: SessionConfig, port: number | null): void {
    ...
    config.systemPrompt = (config.systemPrompt ?? '') + '\n\n' + (step ? this.assignment(r!, step.phase, step.instruction, step.factory) + `\nCurrent inputRevision: ${step.inputRevision ?? 0}. Use this exact value in report_responsibility. A new task message will provide an updated value.` :
      ...
      'For Work cards, give every Task, Goal and Routine a short title that uniquely identifies its concrete subject among similar cards. Preserve user-recognizable identifiers such as repository names, PR numbers, ticket keys, services or environments; for example, “Review upload-functions #10061 and peakflo-web #9602”, not “Review upload and web PRs”. Keep the full request and scope intact; the one-sentence summary is only a readable overview. Keep internal 20x IDs, tools and execution instructions in the details.\n' +
