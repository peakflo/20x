...
export function ImageArtifactView({ artifact, artifactApi }: { artifact: Artifact; artifactApi: ArtifactApi }) {
  ...
  return <ArtifactViewState loading={state.loading} error={state.error} missing={!source}><div className="flex h-full items-center justify-center overflow-auto bg-[#0d1117] p-4"><img src={source || ''} alt={artifact.title} className="max-h-full max-w-full object-contain" /></div></ArtifactViewState>
}
