...
export function HtmlArtifactView({ artifact, artifactApi, onMessage, onLinkClick, refreshTrigger = 0 }: { artifact: Artifact; artifactApi: ArtifactApi; onMessage?: (data: unknown) => void; onLinkClick?: (href: string) => boolean; refreshTrigger?: number }) {
  ...
  return <ArtifactViewState loading={state.loading} error={state.error} missing={html === null}><ArtifactHtmlFrame key={`${artifact.path}:${artifact.reloadTrigger}`} html={html || ''} title={artifact.title} taskId={artifact.taskId} path={artifact.path || ''} files={artifact.files || (artifact.path ? [artifact.path] : [])} readFile={artifactApi.read} onLinkClick={onLinkClick} onMessage={onMessage} /></ArtifactViewState>
}
